 ...
 */
package littleelephantandball;

/**
 *
 * @author Ali
 */
public class LittleElephantAndBall 
{
    ...
    {
        System.out.println(getMinNumberOfTurns("RGBRBRGRGRBBBGRBRBRGBGBBBGRGBBBBRGBGRRGGRRRGRBBBBR"));
        System.out.println(getMinNumberOfTurns("BB"));
        System.out.println(getMinNumberOfTurns("RGBRGB"));
    }
    
    /**
     * Calculates minimum number of turns it takes to get the balls of same 
     * color in a row.
     * 
     * @param s - 
     * @return 
     */
    private static int getMinNumberOfTurns(String s)
    {
        if (s.length() <= 1)
            return 0;
        
        int maxConsecutiveColors = Integer.MIN_VALUE;
        ...
        for (int i = 1; i<s.length(); i++)
        {
            ...
            {
                prevChar = s.charAt(i);
                maxConsecutiveColors = Math.max(maxConsecutiveColors, count);
                count = 1;
            }
        }
        
        if (count > maxConsecutiveColors)
            maxConsecutiveColors = count;
        
        return s.length() - maxConsecutiveColors;        
    }
